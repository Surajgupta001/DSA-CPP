...
bool existInTree(node *root, node *targetNode){
    if(root == nullptr) return false;
    if(root == targetNode) return true;
    return existInTree(root->left, targetNode) or existInTree(root->right, targetNode);
}

node *lowestCommonAncestor(node *root, node *p, node *q){
    if(root == p or root == q) return root;
    else if(existInTree(root->left, p) and existInTree(root->right, q)) return root;
    else if(existInTree(root->right, p) and existInTree(root->left, q)) return root;
    else if(existInTree(root->left, p) and existInTree(root->left, q)) return lowestCommonAncestor(root->left, p, q);
    else return lowestCommonAncestor(root->right, p, q);
}

// Modified code of above code for better readability => both code is same
node *lowestCommonAncestor(node *root, node *p, node *q){
